...
var FASTMATCH = new RegExp(/^[^(?:\*|\/)]*\s*require\(['"]([^'"]+)['"]\)/gm);
...
function Wrap (opts) {
    ...
    this.fastmatch = opts ? opts.fastmatch : false;
...
Wrap.prototype.parseRequire = function(body) {
    if (this.fastmatch) {
        var re = FASTMATCH
          , match
          , deps = [];
        ...
        return {strings:deps, expressions: [  ]}
    } else {
        return this.detective.find(body);
    }
}
...
Wrap.prototype.addEntry = function (file, opts) {
    ...
    try {
        var required = self.parseRequire(body);
    }
    catch (err) {
...
Wrap.prototype.require = function (mfile, opts) {
    ...
    if (!opts.target) {
        if(this.verbose)
           console.log("browserify parse:", file, opts.root);
